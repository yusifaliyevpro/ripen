...
ripen copies exact version commands — e.g. `pnpm add react@19.1.0` rather than `pnpm add react@^19.1.0`. Package managers preserve whatever range prefix (`^`, `~`, or none) is already in your `package.json`, so passing the prefix again in the install command is redundant.

ripen also recognises partial version ranges like `^6` or `^6.2` — not just the standard three-part `major.minor.patch` form.
