...
```
cli.tsx  →  detector.ts  →  fetcher.ts  →  app.tsx  →  build-commands.ts
...
4. **`src/build-commands.ts`** — `buildUpdateCommands` groups selected packages by type (`dependencies`, `devDependencies`, `global`) and returns one `pnpm/npm/bun/yarn add` command **string** per group. It does **not** spawn anything — the commands are copied to the clipboard on confirm (see UI flow below). Actual subprocesses (registry/global queries, `gh auth token`) are spawned via `src/lib/run.ts`, a thin `nano-spawn` wrapper.
